...
To overcome these limitations, we developed **MedS-Ins**, a large-scale instruction tuning dataset tailored for medicine. **MedS-Ins** comprises 58 medically oriented language corpora, totaling 5M Instances with 19K Instructions (Notably, In version 1, we counted different combinations of instructions and instances as a unique sample, so we counted 13.5M samples, but now we have expanded the number of our instructions and counted them separately.) samples across 122 tasks. To demonstrate the dataset's utility, we conducted a proof-of-concept experiment by performing instruction tuning on a lightweight, open-source medical language model. The resulting model, **MMedIns-Llama 3**, significantly outperformed existing models across nearly all clinical tasks.
