"""
@Description :   Generate JSON data for hospitalization prediction from MIMIC-IV EHR dataset.
@Author      :   Henrychur 
...
def initialize_result_dict():
    """
    Initializes the result dictionary with metadata for the hospitalization prediction dataset.
    ...
    return {
        ...
        "Definition": [
            "Next I will provide EHR data of a patient. Your goal is to predict whether the patient needs hospitalization. "
        ...
        "Domains": ["Medicine", "EHR", "Hospitalization", "Emergency Department"],
        "Positive Examples": [],
        "Negative Examples": [],
        "Instances": []
    }
...
def extract_instances(data, variables, outcome_col):
    ...
    for i in tqdm(range(len(data)), desc="Processing instances"):
        sample = data.iloc[i]
        # Generate input text with key-value pairs from the specified variables
...
if __name__ == "__main__":
    ...
    output_file_path = "task104_mimic4ed_benchmark_hospitalization.json"
    
    # Define the variable columns to be included in the input data
    variables = [
        "age", "gender",
    ...
    outcome_column = "outcome_hospitalization"
